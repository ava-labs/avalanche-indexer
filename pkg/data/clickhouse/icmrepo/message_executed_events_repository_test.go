...
func TestMessageExecutedEvents_WriteMessageExecutedEvent_Success(t *testing.T) {
	...
	mockConn.
		On("Exec", mock.Anything, mock.MatchedBy(func(q string) bool {
			return containsSubstring(q, "INSERT INTO") && containsSubstring(q, "`icm`.`icm_message_executed_events`")
		}),
			testBlockchainID,
			"43114",
			uint64(100),
...
func TestMessageExecutedEvents_DeleteMessageExecutedEvents_Success(t *testing.T) {
	...
	mockConn.
		On("Exec", mock.Anything,
			"DELETE FROM `icm`.`icm_message_executed_events_local` ON CLUSTER 'default' WHERE evm_chain_id = ?\n",
			chainID,
		).
		Return(nil).
		Once()
