-- Manual runbook: add the by_message_id projection to icm_add_fee_events_local.
--
...
ALTER TABLE `<database>`.`icm_add_fee_events_local` ON CLUSTER `<cluster>`
...
-- WHERE table = 'icm_add_fee_events_local' AND command LIKE '%by_message_id%';
