...
func TestRelayerRewardRedeemedEvents_WriteRelayerRewardRedeemedEvent_Success(t *testing.T) {
	...
	mockConn.
		On("Exec", mock.Anything, mock.MatchedBy(func(q string) bool {
			return containsSubstring(q, "INSERT INTO") && containsSubstring(q, "`default`.`icm_relayer_reward_redeemed_events`")
		}),
			testBlockchainID,
			"43114",
			uint64(100),
...
func TestRelayerRewardRedeemedEvents_DeleteRelayerRewardRedeemedEvents_Success(t *testing.T) {
	...
	mockConn.
		On("Exec", mock.Anything,
			"DELETE FROM `default`.`icm_relayer_reward_redeemed_events_local` ON CLUSTER 'default' WHERE evm_chain_id = ?\n",
			chainID,
		).
		Return(nil).
		Once()
