...
func TestMessageExecutionFailedEvents_WriteMessageExecutionFailedEvent_Success(t *testing.T) {
	...
	mockConn.
		On("Exec", mock.Anything, mock.MatchedBy(func(q string) bool {
			return containsSubstring(q, "INSERT INTO") && containsSubstring(q, "`default`.`message_execution_failed_events`")
		}),
			testBlockchainID,
			"43114",
			uint64(100),
...
func TestMessageExecutionFailedEvents_DeleteMessageExecutionFailedEvents_Success(t *testing.T) {
	...
	mockConn.
		On("Exec", mock.Anything,
			"DELETE FROM `default`.`message_execution_failed_events_local` ON CLUSTER 'default' WHERE evm_chain_id = ?\n",
			chainID,
		).
		Return(nil).
		Once()
