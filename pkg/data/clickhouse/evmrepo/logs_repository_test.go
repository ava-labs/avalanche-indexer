...
func TestLogsRepository_WriteLog_Success(t *testing.T) {
	...
	for i, topic := range log.Topics {
		...
		require.NoError(t, err, "topic conversion should succeed")
	...
	mockConn.
		On("Exec", mock.Anything, mock.MatchedBy(func(q string) bool {
			...
			*log.BlockchainID,         // string: blockchain ID
			...
			topicsBytes,               // []string: array of 32-byte binary strings
...
func TestLogsRepository_WriteLog_EmptyTopics(t *testing.T) {
	t.Parallel()
	mockConn := &testutils.MockConn{}
	ctx := t.Context()

	// Create a test log with empty topics
	log := createTestLog()
	log.Topics = []string{}
	...
	mockConn.
		On("Exec", mock.Anything, mock.MatchedBy(func(q string) bool {
			...
			*log.BlockchainID,         // string: blockchain ID
			...
			log.TxIndex,               // uint32
			string(addressBytes[:]),   // string: 20-byte binary string
			topicsBytes,               // []string: empty array
			string(log.Data),          // string: binary data
			log.LogIndex,              // uint32
			log.Removed,               // uint8
		).
...
func createTestLog() *LogRow {
	...
	return &LogRow{
		...
		Topics: []string{
			...
			"0x0000000000000000000000004142434445464748494a4b4c4d4e4f5051525354", // from address
			"0x00000000000000000000000055565758595a5b5c5d5e5f6061626364656667",   // to address
		},
