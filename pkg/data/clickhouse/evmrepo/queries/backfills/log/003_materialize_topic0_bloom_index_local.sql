...
--
-- Migration 003 adds idx_topic0_bloom, idx_topic1_bloom, idx_topic2_bloom and
-- idx_topic3_bloom to raw_logs_local (topic0 = event signature; topic1/2/3 hold
-- the indexed from/to party addresses of transfer events); new and merged parts
-- pick them up automatically, but pre-existing parts are not indexed until this
...
ALTER TABLE `<database>`.`raw_logs_local` ON CLUSTER `<cluster>`
MATERIALIZE INDEX idx_topic1_bloom;

ALTER TABLE `<database>`.`raw_logs_local` ON CLUSTER `<cluster>`
MATERIALIZE INDEX idx_topic2_bloom;

ALTER TABLE `<database>`.`raw_logs_local` ON CLUSTER `<cluster>`
MATERIALIZE INDEX idx_topic3_bloom;
...
-- WHERE table = 'raw_logs_local' AND command LIKE '%idx_topic%_bloom%';
