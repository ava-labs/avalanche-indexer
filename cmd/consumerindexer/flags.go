...
func runFlags() []cli.Flag {
	return []cli.Flag{
		...
		&cli.IntFlag{
			...
			Usage:   "Maximum number of concurrent goroutines the batch writer uses to flush batches to ClickHouse",
			EnvVars: []string{"BATCH_WRITER_WORKERS"},
			Value:   3,
		},
		&cli.IntFlag{
			Name:    "batch-writer-max-blocks",
			Usage:   "Maximum number of blocks the batch writer dispatcher accumulates before triggering a flush",
			EnvVars: []string{"BATCH_WRITER_MAX_BLOCKS"},
			Value:   1000,
		},
		&cli.DurationFlag{
			Name:    "batch-writer-flush-timeout",
			Usage:   "Maximum time the batch writer dispatcher waits before flushing an incomplete batch",
		...
		&cli.StringFlag{
			Name:    "icm-receipt-events-table-name",
			Usage:   "ClickHouse table name for ICM receipt received events",
			EnvVars: []string{"ICM_RECEIPT_EVENTS_TABLE_NAME"},
			Value:   "icm_receipt_events",
		},
		&cli.StringFlag{
			Name:    "icm-add-fee-events-table-name",
			Usage:   "ClickHouse table name for ICM add fee events",
			EnvVars: []string{"ICM_ADD_FEE_EVENTS_TABLE_NAME"},
			Value:   "icm_add_fee_events",
		},
		&cli.StringFlag{
			Name:    "icm-relayer-reward-redeemed-events-table-name",
			Usage:   "ClickHouse table name for ICM relayer reward redeemed events",
			EnvVars: []string{"ICM_RELAYER_REWARD_REDEEMED_EVENTS_TABLE_NAME"},
			Value:   "icm_relayer_reward_redeemed_events",
		},
	}
}
