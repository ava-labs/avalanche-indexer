...
func remove(c *cli.Context) error {
	...
	rawBlocksRepo, err := evmrepo.NewBlocks(ctx, chClient, rawBlocksTableName)
	if err != nil {
		return fmt.Errorf("failed to create blocks repository: %w", err)
	}
	rawTransactionsRepo, err := evmrepo.NewTransactions(ctx, chClient, rawTransactionsTableName)
	if err != nil {
		return fmt.Errorf("failed to create transactions repository: %w", err)
	}
	rawLogsRepo, err := evmrepo.NewLogs(ctx, chClient, rawLogsTableName)
	if err != nil {
